...
func (c *Criteria) Apply(bCtx *env.BubblyContext, ctx *core.ResourceContext) core.ResourceOutput {
	...
	if err := json.Unmarshal(bytes, &result); err != nil {
		return core.ResourceOutput{
			ID:     c.ID(),
			Status: events.ResourceRunFailure,
			Error:  fmt.Errorf("error unmarshalling GraphQL result: %w", err),
			Value:  cty.NilVal,
		}
	}
	if result.HasErrors() {
		return core.ResourceOutput{
			ID:     c.ID(),
			Status: events.ResourceRunFailure,
			Error:  fmt.Errorf("error running graphql query: %v", result.Errors),
			Value:  cty.NilVal,
		}
	}
	...
	for _, condition := range c.Spec.Conditions {
		...
		if !resultVal.Type().Equals(cty.Bool) {
			return core.ResourceOutput{
				ID:     c.ID(),
				Status: events.ResourceRunFailure,
				Error:  errors.New("condition is not boolean type"),
				Value:  cty.NilVal,
			}
		}
		// If the condition was not met, then we can stop here and return the reason
		if resultVal.False() {
			result := core.CriteriaResult{
				Result: false,
				Reason: condition.Message,
			}
			ty, err := gocty.ImpliedType(result)
			if err != nil {
				return core.ResourceOutput{
					ID:     c.ID(),
					Status: events.ResourceRunFailure,
					Error:  fmt.Errorf("error getting implied type: %w", err),
					Value:  cty.NilVal,
				}
			}
			ctyResult, err := gocty.ToCtyValue(result, ty)
			if err != nil {
				return core.ResourceOutput{
					ID:     c.ID(),
					Status: events.ResourceRunFailure,
					Error:  fmt.Errorf("error getting cty result: %w", err),
					Value:  cty.NilVal,
				}
			}
			return core.ResourceOutput{
	...
	// If all conditions have been evaluated and they all succeeded, then we can
	// return saying that this criteria has succeeded
	return core.ResourceOutput{
		ID:     c.ID(),
		Status: events.ResourceRunSuccess,
		// TODO: passed!
